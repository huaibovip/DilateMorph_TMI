...
class DiceLoss(nn.Module):
    ...
    def __init__(self, one_hot=False, num_classes=-1, loss_weight=1.0):
        super().__init__()
        if one_hot:
            assert num_classes > 0, 'num_classes must > 0'
    ...
    def forward(self, pred: Tensor, true: Tensor):
        if self.one_hot:
            pred = F.one_hot(torch.round(pred).long(), self.num_classes)
            pred = pred.squeeze_(1).permute(0, 4, 1, 2, 3).contiguous()
            true = F.one_hot(true.long(), self.num_classes)
            true = true.squeeze_(1).permute(0, 4, 1, 2, 3).contiguous()

        intersection = (pred * true).sum(dim=[2, 3, 4])
        union = (
            torch.pow(pred, 2).sum(dim=[2, 3, 4]) +
            torch.pow(true, 2).sum(dim=[2, 3, 4]))
        loss = (2.0 * intersection) / (union + 1e-5)
