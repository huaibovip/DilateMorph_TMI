...
def window_partition(x, window_size):
    ...
    shape = (-1, window_size[0], window_size[1], C)
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(*shape)
    return windows
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_dim=3, embed_dim=96, norm_layer=None):
        super().__init__()
...
class TransMorph2d(nn.Module):
    ...
    def __init__(
        ...
        self.c2 = Conv2dReLU(2, flow_dim, 3, 1, use_batchnorm=False)
...
@MODELS.register_module()
class TransMorph2dHalf(nn.Module):
