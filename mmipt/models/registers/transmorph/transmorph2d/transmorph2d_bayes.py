...
from . import transmorph2d as TM
from ..transmorph_bayes import Mlp
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        relative_pos_index = relative_coords.sum(-1)  # Wh*Ww, Wh*Ww
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                shape = (-1, H, W, self.num_features[i])
                out = x_out.view(*shape).permute(0, 3, 1, 2).contiguous()
                outs.append(out)
        return outs
...
class TransMorph2dBayes(nn.Module):
    ...
    def __init__(
        ...
        super(TM.TransMorph2d, self).__init__()
        self.mc_drop = mc_drop

        self.avg_pool = nn.AvgPool2d(3, stride=2, padding=1)
        self.c1 = TM.Conv2dReLU(2, embed_dim // 2, 3, 1, use_batchnorm=False)
        self.c2 = TM.Conv2dReLU(2, flow_dim, 3, 1, use_batchnorm=False)
    ...
    def forward(self, source, target, **kwargs):
        ...
        f4 = self.c1(x_s1)
        f4 = F.dropout2d(f4, self.mc_drop, training=True)
        f5 = self.c2(x_s0)
        f5 = F.dropout2d(f5, self.mc_drop, training=True)
