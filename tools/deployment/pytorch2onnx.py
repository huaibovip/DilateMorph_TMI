...
from mmipt.apis import show_result_pyplot
from mmipt.apis.inference import LoadImage
from mmipt.datasets.pipelines import Compose
from mmipt.models import build_register
from mmipt.ops import resize
...
def parse_args():
    parser = argparse.ArgumentParser(description='Convert MMipt to ONNX')
    parser.add_argument('config', help='test config file path')
    parser.add_argument('--checkpoint', help='checkpoint file', default=None)
    parser.add_argument(
        '--moving-img', type=str, help='Moving images', default=None)
    parser.add_argument(
        '--fixed-img', type=str, help='Fixed images', default=None)
    parser.add_argument(
        '--show',
        action='store_true',
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help='input image depth, height and width.')
    parser.add_argument(
        '--rescale_shape',
        type=int,
        nargs='+',
...
if __name__ == '__main__':
    ...
    if args.shape is None:
        ...
        input_shape = (1, 1, img_scale[1], img_scale[0])
    elif len(args.shape) == 1:
        input_shape = (1, 1, args.shape[0], args.shape[0])
    ...
    register = _convert_batchnorm(register)

    if args.checkpoint:
        checkpoint = load_checkpoint(
            register, args.checkpoint, map_location='cpu')
        register.CLASSES = checkpoint['meta']['CLASSES']
        register.PALETTE = checkpoint['meta']['PALETTE']
    ...
    if args.input_img is not None:
        ...
        if isinstance(register.decode_head, nn.ModuleList):
            num_classes = register.decode_head[-1].num_classes
        else:
            num_classes = register.decode_head.num_classes
