...
def move(data_root, splits, delete=True):
    # splits = dict(train=(0, 201), val=(201, 213), test=(213, 414))
    # from mmengine.fileio import dump
    paths = glob.glob(osp.join(data_root, 'OASIS_OAS1_*'))
    for phase in splits.keys():
        os.makedirs(osp.join(data_root, phase), exist_ok=True)
        left, right = splits[phase]
        move_cfg = [
            dict(
                src=osp.join(path, 'slice_norm.nii.gz'),
                dst=osp.join(
                    data_root, phase,
        ...
        move_cfg += [
            dict(
                src=osp.join(path, 'slice_seg24.nii.gz'),
                dst=osp.join(
                    data_root, phase,
    ...
    # delete
    if delete:
        delete_path = glob.glob(osp.join(data_root, 'OASIS_OAS1_*'))
...
def struct_data(paths, phase):
    if phase == 'train':
        ...
        zip_filename_1 = list(zip(paths[:-1], paths[1:]))
        zip_filename_2 = list(zip(paths[1:], paths[:-1]))
        filenames = zip_filename_1 + zip_filename_2
    return filenames
...
def write_ann(file, prefix, paths):
    ...
    data_ann = dict()
    data_ann['metainfo'] = get_metainfo('oasis2d')
...
if __name__ == '__main__':
    # dataset: https://github.com/adalca/medical-datasets/blob/master/neurite-oasis.md
    data_link = 'https://surfer.nmr.mgh.harvard.edu/ftp/data/neurite/data/neurite-oasis.2d.v1.0.tar'
    root = 'data/reg/OASIS2d'
    data_root = osp.join(root, 'datas')
    splits = dict(train=(0, 201), val=(201, 213), test=(213, 414))

    mkdir_or_exist(data_root)
    download(data_link, dir=data_root, delete=True)
    ...
    for phase in splits.keys():
        ...
        write_ann(
            osp.join(root, 'annotations', f'{phase}_oasis2d_scan2scan.json'),
